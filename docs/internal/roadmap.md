...
- **Subagent `tools:` frontmatter audit** — each of the 10 specialists in `Plugin~/agents/` declares its MCP tools in the prose body ("## MCP Tools Available" section) but currently lists only built-ins (`Read, Glob, Grep, Write, Edit, Bash`) in the YAML `tools:` frontmatter. Subagents only receive the schema for tools listed in `tools:` at spawn time — they do **not** have access to `ToolSearch` like the main agent does — so the body text is misleading: a specialist invoked via `Task` cannot actually call the MCP tools its prompt promises. Audit each specialist, reconcile the body's tool list with the YAML frontmatter, and add the relevant `mcp__game-deck__<tool>` entries. Example: `unity-shader-specialist` should list `mcp__game-deck__shader-list, shader-inspect, vfx-list-particle-systems, graphics-get-settings, graphics-stats, texture-create, texture-configure, texture-inspect, material-create, material-get-info, material-update, profiler-toggle, profiler-frame-timing, profiler-frame-debugger-enable, screenshot-camera, screenshot-gameview, screenshot-sceneview` in the YAML. **This is plugin-side polish, not supervisor wiring** — can be advanced into v2.0 if desired (no F02 dependency). Discovered during F02 task 3.4 validation.
...
**Just shipped:** Feature 07 (Editor Status Pin) — merged to `develop/v2.0` April 2026. The pin replaces the in-Unity `Editor/ChatUI/` panel with a `MainToolbarDropdown` entry that polls MCP server status (gray/red/yellow/green/badge), launches Tauri via `Process.Start` with the env-var contract, and surfaces a 5-item dropdown menu (Open Chat / Settings / Copy URL / Show install folder / About). Tauri side gained `tauri-plugin-single-instance`, `tauri-plugin-cli`, and `tauri-plugin-opener` plus an `UpdateBanner` component. Single-instance is app-global (per-project isolation deferred to v2.1+ — the official plugin doesn't expose runtime ID injection). Validation of the SUCCESS path (real download → spawn → connected app) deferred to v2.0 release rehearsal alongside several 4.x/5.x items — needs a real GitHub release with `.exe` + `.sha256` assets.
