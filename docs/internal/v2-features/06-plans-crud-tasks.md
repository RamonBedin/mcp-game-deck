...
- **F02 bug: supervisor does not preserve `session_id` between turns.** Each call to `handleInput` in `App~/runtime/sdk-entry.js` invokes a fresh `query()`. The SDK's `resume` option is only set when the Rust side explicitly sends `setResumeSession` (which happens only when the user clicks an existing session in the sidebar). Within the same logical session, the supervisor never captures the `session_id` from the SDK's `init` event and never propagates it to the next turn — so every turn is an isolated SDK session. Claude has no memory of prior turns in the same conversation. This is a regression of multi-turn conversations introduced by F02, not a defect of F06. Discovered during F06 task 3.1 when `/save-plan` consistently fell back to Tier 3 (AskUserQuestion) because the skill's view of "the conversation" had nothing before the slash command itself. **Status: fixed in this branch (`feature/06-plans-crud`) as an isolated commit before resuming task 3.1.** The fix captures `session_id` inside `handleInput`'s `for await` loop from any `system/init` or `result` SDK message and updates `pendingResumeSessionId` so the next turn resumes the same session. Source-of-truth at `App~/src-tauri/src/claude_supervisor/sdk_entry.js` (the `runtime/` copy is a regenerated artifact).

- **`@agent-` literal prefix returns empty dropdown (spec premise mismatch).** The F06 spec for task 6.4 proposed `"Type @agent- → only agents (files all filtered out by literal-substring on path)"`. The implementation of `useAtAutocomplete` filters agents by `name.includes(query)` OR `description.includes(query)`, but agent names in the catalog are bare (`Explore`, `general-purpose`, `mcp-game-deck:gameplay-programmer`, etc) — the `agent-` prefix only exists in the *insertion* shape (`@agent-<name> `), not in the `name` property. Result: query `"agent-"` matches zero agents and zero files, dropdown shows nothing. Functionally fine — the user types the actual agent name (`@unity`, `@gameplay`) for autocomplete — but the empty result is silent. F09 polish options: (a) special-case the `agent-` literal prefix in the filter and treat the suffix as the real query against agent names; (b) render an empty state with the hint "Agent names don't include the `agent-` prefix — try the bare name". Discovered during F06 task 6.4 smoke.

- **Files watcher fires `project-files-changed` for mutations inside excluded directories.** The `notify-debouncer-mini` watch in `files_watcher.rs` is rooted at `<project>/` recursively=true with no path-level exclusion; exclusions only kick in during the walkdir rebuild inside `commands::files::walk_project`. Result: changes inside `Library/`, `Temp/`, `obj/`, `Logs/`, `node_modules/`, `.git/` still trigger an event → React refetches → walkdir filters those entries out → index stays clean. The on-disk index is never polluted, but during Unity reimports / asset bakes (which churn Library/ heavily) the React side will refetch frequently for no semantic change. Smoke during F06 task 6.1 caught two such phantom events while testing Library/ writes. v2.0 acceptable per spec ("watched but discarded; OK either way as long as it doesn't pollute the index"). F09 polish: pre-filter at the notify layer using the same exclusion set as `should_exclude`, so excluded subtrees never reach the debouncer. Discovered during F06 task 6.1 smoke.

- **Known limitations of the session_id capture fix (Option A — JS-side only).** The fix above does NOT propagate the captured `session_id` back to the Rust supervisor's `current_resume_session_id` state. Two consequences:
  - **Supervisor restart drops the captured session.** After a crash recovery or `restart_supervisor` call, Rust re-pushes the last id it knows about (the one React explicitly resumed, or none) via the spawn-time re-sync at `claude_supervisor::mod.rs:395-403`, overwriting the in-memory captured id. The conversation forks a new session on the next turn. Restart is rare, so accepted for v2.0; full fix is Option B — emit a `session-changed` envelope from JS to Rust, mirroring the existing `permission-mode-changed` echo path (new `AgentMessage` variant + `read_stdout` branch + emit helper).
  - **Edge case: "+ New Session" mid-flight.** If the user sends message A, then clicks "+ New Session" before A's response arrives, then sends message B: Rust's `clearResumeSession` lands on stdin and zeroes `pendingResumeSessionId` while A is still in-flight; A's `result` message then captures A's session_id, overwriting the null; B starts and uses that id, resuming the old session instead of starting fresh. Probability is low (user rarely clicks +New mid-stream); recovery is trivial (click +New again). Not fixed in this commit; the structural fix is to gate the capture on whether `clearResumeSession` arrived since the current `query()` started — left for a later cleanup.
