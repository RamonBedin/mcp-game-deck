...
| 3.5 | Block integration — conversationStore "request" branch + ChatRoute BlockView routing | S | ✅ | 2026-05-06 | + AskUserQuestionOutput wire-shape fix (was bug from 1.2/2.3 spec): SDK expects `{ questions, answers: Record<string, string> }`, not `{ answers: Array<{selectedOptions, freeTextResponse}> }`. Without `questions` echoed, SDK throws `H.map is not a function`. Caught during smoke 4.3 dry-run; fixed in `types.ts` + `QuestionCard.tsx` (handleSubmit constructs new shape; multi-select concatenates with `, `; answered view simplified to show answer string verbatim instead of reconstructing selections). **Group 3 closed (5/5).** F04 ALIVE: cards render inline, round-trip end-to-end. |
| 4.1 | Lifecycle — supervisor crash → mark all pending requests interrupted | S | ✅ | 2026-05-06 | Discovered during smoke: design Decision #7 was wrong about "automatic re-emit on resume". SDK reality is that `resume: sessionId` only applies on the next `query()` call, which requires a new user message after reconnect. Orphaned tool calls without follow-up message stay stranded. Decision #7 in design doc updated to reflect actual SDK behavior. 4.1 itself works as specified — cards do flip to interrupted on crash. |
| 4.2 | Smoke — main thread permission card round-trip | S | ✅ | 2026-05-06 | 5 cenarios validados: Read+Allow / Read different input+Allow Always / cache hit (auto-allowed compact line) / Bash+Deny / MCP scene tool+Allow. Visual + wire OK. |
| 4.3 | Smoke — main thread question card (multi-question + multi-select + free-text) | S | ✅ | 2026-05-06 | Single-select / multi-question / free-text all validated. **Discovered:** the SDK does NOT auto-inject "Other (specify)" like Claude Code CLI does (CLI's internal system prompt forces it; SDK is deliberately raw). Workaround applied in `sdk-entry.js`: `augmentAskUserQuestionInput` injects "Other (specify)" into every question's options before emitting to React, then strips it back to the original shape before returning `updatedInput` to the SDK. React's existing `isFreeTextOption` heuristic detects the injected label and renders the text input naturally. Skips injection if Claude already included an Other-style option. |
| 4.4 | Smoke — subagent permission + question cards (Decision #5 validation) | M | ✅ | 2026-05-06 | Decision #5 confirmed: subagents inherit `canUseTool` callback (permission cards surface with `agentId` populated, RequestCard's "from <agentId>" indicator appears) and can call `AskUserQuestion` (question card surfaces from subagent context too). All 4 cenarios passed: Read+permission / Bash+permission / MCP tool+permission / AskUserQuestion via subagent. |
| 4.5 | Cleanup — delete F05 docs, update roadmap.md, update v2-features README.md | S | ✅ | 2026-04-XX | Done early during F04 trio creation when F04+F05 were unified. F05 docs deleted, roadmap.md notes "absorbs former F05", README.md lists `04-interactive-approvals* ← design locked (absorbs former F05)`. |
