...
- On `supervisor-status-changed: ready` after a manual restart, no re-emit of the original request — Claude Code's session storage retains the pending tool call and replays it on session resume. The user clicks the latest session in `SessionList`, the supervisor restarts, the same tool call (or a new equivalent) re-fires `canUseTool`, a fresh `RequestCard` appears.
