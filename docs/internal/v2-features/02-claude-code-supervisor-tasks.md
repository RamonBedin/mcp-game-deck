...
| 3.1 | Asset surfacing — Plugin~/ surfaced via SDK `plugins` option | S | ✅ | 2026-04-30 | Pivot mid-task após teste empírico revelar que `additionalDirectories + settingSources: ['project']` NÃO dispara skill auto-discovery via SDK — SDK só escaneia `<cwd>/.claude/skills/` e plugins registrados. Trocado para mecanismo `plugins: [{type:"local", path:...}]` do SDK que é o caminho oficial pra distribuir skills empacotados. Pós-pivot, decidido também **renomear `Skills~/` → `Plugin~/`** porque o diretório agora hospeda mais que skills (manifest + skills + agents na 3.2). Mudanças aplicadas: (1) reestruturado `Plugin~/` (renomeado de `Skills~/`) com `Plugin~/.claude-plugin/plugin.json` (manifest do plugin com name=mcp-game-deck, version=1.1.0, MIT) e movendo as 22 skills pra `Plugin~/skills/<name>/SKILL.md` conforme convenção de plugin; (2) sdk_entry.js: novo helper `buildPlugins()` retorna `[{type:'local', path: pluginDir}]` lendo MCP_GAME_DECK_PLUGIN_DIR; `buildAdditionalDirectories()` simplificado pra só commands (filesystem read access, não discovery); removido `settingSources: ['project']` das options; (3) paths.rs: `skills_dir()` virou `plugin_dir()` apontando pra `package_root().join("Plugin~")`; (4) spawn.rs: `resolve_skills_dir` virou `resolve_plugin_dir`, env var `MCP_GAME_DECK_SKILLS_DIR` virou `MCP_GAME_DECK_PLUGIN_DIR`. **CRITÉRIO PRINCIPAL VALIDADO**: prompt "List all available skills" no chat retornou 22 skills com namespace `mcp-game-deck:` (architecture-decision, code-review, asset-audit, ...) + 10 built-ins do Claude Code = 32 total. Smoke válido com Unity aberto + env vars setadas + supervisor: ready. **Implicações nas tasks futuras**: 3.2 reescopada de "copy Agents~/ → .claude/agents/gamedeck-*.md with {{KB_PATH}}" pra "consolidate Agents~/ into Plugin~/agents/" (plugin nativamente suporta agents via subdir agents/, mesmo namespacing). 3.3 removida da status table — plugin não copia nada, manifest tracking e uninstall menu ficam sem propósito. Decision #2/#4/#7 da F02 obsoletas em parte ({{KB_PATH}} substitution ainda relevante se algum agent referencia paths absolutos). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 3.2 | Asset surfacing — plugin self-contained com `${CLAUDE_PLUGIN_ROOT}/knowledge` | M | ✅ | 2026-04-30 | Pivot Caminho A → C após teste empírico revelar que o copy step do Caminho A (asset_install.rs copiando Plugin~/ pra Library/GameDeck/plugin/ com substituição de `{{KB_PATH}}` pelo path absoluto do `<package>/KnowledgeBase~/`) **não resolvia o problema**: mesmo com substitution funcionando, Claude Code SDK bloqueava Read em paths fora do working directory ("Claude Code may only list files in the allowed working directories: G:\Unity-Projects\Jurassic\Jurassic"). KnowledgeBase~/ era irmão do Plugin~/ no package, fora do escopo permitido. Solução final (Caminho C): plugin self-contained com KB dentro. **Mudanças aplicadas:** (1) `KnowledgeBase~/` movido pra `Plugin~/knowledge/` (16 .md); (2) find-and-replace `{{KB_PATH}}` → `${CLAUDE_PLUGIN_ROOT}/knowledge` em 9 agents (todos os specialists exceto technical-artist que já usava bare references) + 3 skills (new-feature, new-system, refactor); (3) reversão do Caminho A: deletado `asset_install.rs`, removidos helpers `knowledge_base_dir()` e `installed_plugin_dir()` de paths.rs, restaurada função interna `resolve_plugin_dir()` em spawn.rs apontando direto pro source `Plugin~/`, removido `pub mod asset_install;` e bloco do install_plugin em mod.rs::spawn(); (4) **descoberta crítica durante validação:** SDK `plugins` option não auto-grant filesystem read access pro plugin dir — plugins option faz só discovery, working-directory restriction continua aplicando. Fix: adicionado `MCP_GAME_DECK_PLUGIN_DIR` em `additionalDirectories` em sdk_entry.js (redundância intencional: plugins=discovery + additionalDirectories=read access). Doc comment do buildPlugins atualizado explicando o aprendizado empírico (não estava na doc oficial do `${CLAUDE_PLUGIN_ROOT}`). **CRITÉRIO PRINCIPAL VALIDADO:** prompt "Use the unity-shader-specialist agent to read its knowledge base and tell me what topics it has" → agent invocado, Read pediu permission default mode, approved, **leu conteúdo real** dos 3 KB docs primários (13-audio-vfx-systems.md, 06-mobile-optimization.md, 11-asset-pipeline-addressables.md) plus listou os 16 .md via Glob. Sem mais working directory error. Também: "List all available skills" retornou 22 skills `mcp-game-deck:*` + 10 built-ins (regressão zero da 3.1); "What agents are available" retornou 10 specialists `@agent-mcp-game-deck:*` + 4 built-ins. **Bugs colaterais descobertos durante validação** (commit separado de first-run polish): (a) `runtime_setup::ensure_entry_script` não criava parent dir defensivamente — falha em PC fresh sem App~/runtime/ (gitignored); fix: `tokio::fs::create_dir_all(parent)` antes do write; (b) `install_check.rs` usava `Command::new("claude")` direto em Windows que não resolve `.cmd` shims do npm global — install-check spammava "program not found" mesmo com claude funcional; fix: `cmd /C claude --version` em Windows seguindo o pattern já usado pelo sdk_install.rs. Bugs latentes registrados pra 7.x polish: bootstrap race (supervisor failed na primeira spawn, ready após restart manual), sdk-entry.js stale cache (template fonte muda mas runtime não regenera até apagar manualmente). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 3.4 | Empirical smoke test — agent + MCP tool wiring validado end-to-end | S | ✅ | 2026-04-30 | Validada via 3 evidências complementares: (1) **Decision #3 validada implicitamente na 3.2**: o agent `@agent-mcp-game-deck:unity-shader-specialist` foi invocado, executou tool calls (Read + Glob built-ins) com sucesso, retornou conteúdo real dos KB docs. Subagents conseguem fazer tool calls sem bater no bug do issue #25200. (2) **Smoke específico de MCP tool tentado pelo subagent (1ª tentativa)**: prompt "Use unity-shader-specialist agent to call shader-list MCP tool" — agent invocado OK mas tentou `mcp__game-deck__shader-list` e recebeu "No such tool available" mesmo com Unity ativo + mcp-proxy buildado + supervisor ready. Agent foi resiliente: caiu pra Glob filesystem direto e produziu relatório estruturado de 210+ shaders. (3) **Investigação pela Claude principal (2ª tentativa)**: descobriu que subagents recebem só as MCP tools "ativas" no momento da spawn — não têm acesso ao ToolSearch pra carregar deferred tools. A Claude principal usa ToolSearch on-demand: chamou `ToolSearch(query: "select:mcp__game-deck__console-get-logs")` → schema carregou → tentou invocar `mcp__game-deck__console-get-logs` → permission system pediu approval (default mode esperado, comportamento correto). **MCP server confirmadamente rodando e tools registradas.** **DESCOBERTA TÉCNICA IMPORTANTE:** subagents (agents do plugin invocados via Task) precisam ter as MCP tools listadas no campo `tools:` do frontmatter pra tê-las disponíveis, OU usar só built-ins (Read/Glob/Bash). Os 10 specialists hoje só listam built-ins (`Read, Glob, Grep, Write, Edit, Bash`), por isso não conseguiram chamar `mcp__game-deck__*` direto. Opções pra futuro: (a) adicionar MCP tools relevantes ao frontmatter de cada specialist (ex: shader-specialist ganha `mcp__game-deck__shader-list, shader-inspect, vfx-list-particle-systems, ...`); (b) deixar subagents focados em pesquisa via built-ins e chamar MCP tools só pelo agente principal. Esse design não foi tocado nessa task — fica como decision pendente pra Group 4 ou política do plugin. Também: a sessão exposou que os 250+ MCP tools ficam todos como deferred até ToolSearch select — padrão esperado do Claude Code pra economia de schema overhead. **Conclusão final:** pipeline `agente principal → MCP tool` 100% validado (com permission step esperado pra 4.2 resolver). Pipeline `subagent → MCP tool` requer decisão de design sobre frontmatter `tools:` — fora do escopo da 3.4. Bug colateral pra 7.x polish: agent gerou comando bash com paths com espaços sem quoting ("Assets\TextMesh Pro\..."), permission system rejeitou interpretando como "cd com 2 args". |
| 4.1 | `send_message` Tauri command rewritten to drive Agent SDK input | S | ✅ | 2026-04-30 | Refactor de 6 arquivos aplicado pelo Claude Code dev. **Rust side**: `commands/conversation.rs::send_message` agora aceita `(text: String, attachment_paths: Vec<String>)`, forwards via `supervisor.send_input(&text, &attachment_paths)`. `claude_supervisor/mod.rs::send_input` mudou assinatura pra `(&self, text: &str, attachments: &[String])`, JSON serializado pro stdin agora carrega `{"type":"input","text":"...","attachments":[...]}` (array vazio enquanto Group 5 não liga handling real). `lib.rs` removeu `commands::conversation::get_conversation_history` e `clear_conversation` da macro `tauri::generate_handler!`. **JS side**: `sdk_entry.js` (template + runtime mirror) lê `parsed.attachments` default [] no stdin loop, passa pra `handleInput(text, attachments)` que loga via `debug()` ("attachment handling lands in Group 5"); `query()` continua com `prompt: text` (round-trip preservado). **TS side**: `src/ipc/commands.ts::sendMessage(text, attachmentPaths: string[] = [])`, removidos exports `getConversationHistory` e `clearConversation` (zero callers em React). `src/stores/conversationStore.ts::sendMessageCommand` passa `[]` internamente; assinatura externa do store inalterada (`sendMessage: (text) => Promise<void>`). **Removidos por superseded:** os 2 stubs vazios de history/clear (Claude Code session storage é source of truth via task 4.4; `/clear` nativo do Claude Code substitui clear_conversation). `set_permission_mode` + `get_permission_mode` mantidos (stubs hoje, implementação real na 4.2). **VALIDADO:** smoke manual em Tauri+Unity: (1) round-trip básico "say hello in pirate" → resposta real ("Ahoy, matey!" + emoji + "Arrr!"); (2) multi-turn "what year is it now? and the next year?" → contexto preservado ("2026... 2027"); (3) tool call "list scenes in unity" → ToolSearch select → `mcp__game-deck__scene-list` invocado → permission ask default mode (esperado, 4.2 resolve). Zero regressão na pipeline 2.4. Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke pra forçar regeneração do template fonte (bug latente B4 sdk-entry stale cache, registrado pra 7.x). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 4.2 | Permission mode UI — dropdown + Tauri commands `get_permission_mode` / `set_permission_mode` | M | ✅ | 2026-04-30 | Plano de 7 camadas aplicado pelo Claude Code dev. **Types**: `PermissionMode` enum 5 variantes camelCase via `#[serde(rename_all = "camelCase")]` (Default | AcceptEdits | Plan | BypassPermissions | Auto); TS literal type matching. **Supervisor** (`claude_supervisor/mod.rs`): novo field `permission_mode: Arc<StdMutex<PermissionMode>>` default Default, `current_permission_mode()` getter, `set_permission_mode(mode)` que (a) atualiza state e (b) escreve `{"type":"setPermissionMode","mode":"..."}` no stdin do JS child (no-op silencioso quando supervisor não está rodando). Em `spawn()`, depois do stdin task armado, envia mode armazenado 1x pra ressincronizar JS após restart (caso supervisor crash + auto-restart preserva escolha). **Tauri commands** (`commands/conversation.rs`): drop dos stubs vazios anteriores; `get_permission_mode(supervisor)` lê do supervisor state, `set_permission_mode(mode, supervisor)` chama `supervisor.set_permission_mode`. **JS** (`sdk_entry.js` template + runtime): `currentPermissionMode` module-level (default `"default"`); helper `resolveSdkMode(m)` mapeia `"auto" → "bypassPermissions"` (gotcha consolidado do CLAUDE.md "Auto permission mode: Uses bypassPermissions, NOT acceptEdits"); resto pass-through. Stdin loop reconhece `{type:"setPermissionMode",mode}` e valida string contra os 5; mode inválido → debug log e skip. `handleInput` passa `permissionMode: resolveSdkMode(currentPermissionMode)` no `query()` options — próximo prompt entra no modo selecionado. **Store** (`conversationStore.ts`): default `"default"` (corrige stub anterior que retornava `"ask"`, variant inválido nos 5 modos do SDK). **UI**: novo `PermissionModeToggle.tsx` com `<select>` 5 modos + label "Permissions"; on mount `getPermissionMode()` → store, on change `setPermissionMode(mode)` → store. Subscription pro evento Shift+Tab fica pra task 4.3 (só React→Rust→JS aqui, não JS→Rust→React). `ChatRoute.tsx` monta toolbar acima do textarea, right-aligned. **VALIDADO:** smoke manual em Tauri+Unity: (1) dropdown renderiza com 5 modos, current selection reflete supervisor state (`default` no first run); (2) switch pra `bypassPermissions` → próximo prompt envolvendo MCP tool ("list scenes in unity") rodou direto **sem permission ask dialog** (compare com 4.1 onde pedia approval em todo tool call — fricção visível desde 2.4 finalmente resolvida via UX user-controllable). Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke pra forçar regeneração do template fonte (bug latente B4 sdk-entry stale cache). **Fora de escopo, registrado:** UI inline pra Allow/Deny individual quando user escolher modo `default` e tool pedir approval — isso é mecanismo separado (permission dialog) que não é F02; provavelmente F05 (Permission system fix) quando retomada, ou nova feature dedicada. Também fora de escopo: persistência cross-Tauri-restart do mode escolhido — hoje permission_mode é in-memory (Arc<StdMutex>); fechar Tauri perde o mode. Adição futura via Settings persist (não bloqueia 4.2). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 4.3 | Permission mode events — Shift+Tab inside chat propagates back to React UI | S | ✅ | 2026-04-30 | Plano de 9 camadas aplicado pelo Claude Code dev. Completa o loop bidirecional do permission mode — 4.2 fez Reaçt→Rust→JS, agora 4.3 fecha JS→Rust→React via Tauri event. **Types** (`types.rs` + `ipc/types.ts`): nova variant `PermissionModeChanged { mode: PermissionMode }` no enum `AgentMessage` (wire format automático `"permission-mode-changed"` via `#[serde(rename_all = "kebab-case")]`); novo struct `PermissionModeChangedPayload { mode }`. **Event channel** (`events.rs` + `ipc/events.ts`): `EVT_PERMISSION_MODE_CHANGED = "permission-mode-changed"` + emitter helper; wrapper `onPermissionModeChanged` no TS. **Rust side** (`claude_supervisor/spawn.rs` + `mod.rs`): `read_stdout` ganhou 4º param `Arc<StdMutex<PermissionMode>>`. Branch para `PermissionModeChanged`: (a) atualiza supervisor field via lock, (b) emite Tauri event pra React. `spawn()` passa `self.permission_mode.clone()` ao `read_stdout`. **JS** (`sdk_entry.js` template + runtime mirror): helper `emitPermissionModeChanged(mode)`. No stdin loop, depois de aplicar `setPermissionMode` com sucesso, emite eco `{type:"permission-mode-changed", mode}` no stdout — Rust recebe via read_stdout, atualiza state, propaga pra React. **UI** (`PermissionModeToggle.tsx` + `ChatRoute.tsx`): toggle ganhou useEffect adicional subscribe a `onPermissionModeChanged`; on event chama `setMode` no store (idempotente — no-op se mode === mode atual, evita bouncing loop quando user originou troca via dropdown). ChatRoute `handleKeyDown` ganhou branch Shift+Tab no textarea: previne default, cicla pro próximo modo (default → acceptEdits → plan → bypassPermissions → auto → default), optimistic update + invoca `setPermissionMode` Tauri — listener confirma via event. **VALIDADO:** smoke manual em Tauri+Unity: (1) Shift+Tab no textarea cicla 5 modos visualmente; dropdown reflete a cada cycle; (2) sync bidirecional dropdown <-> Shift+Tab convergem pro mesmo mode após qualquer ordem de operações; (3) regressão zero da 4.2: switch via dropdown continua propagando pro JS sem regressão. Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke pra forçar regeneração (bug latente B4). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 4.4 | Session list — `get_sessions` Tauri command reading from Claude Code's storage; sidebar UI | M | ✅ | 2026-04-30 | **Opção B (completo) escolhida** sobre A (compacto): chat reload com histórico visível é requisito literal do spec validação #2 ("chat reloads with that session's history"); A deixaria débito técnico imediato. Plano aplicado pelo Claude Code dev. **Rust side**: novo `types.rs::SessionSummary { id, title, last_modified, message_count }`; `claude_supervisor/paths.rs::claude_sessions_dir(project_path)` computa `<home>/.claude/projects/<encoded>/` (encoding `:` `\` `/` → `-`, ex: `E:\Projects\mcp-game-deck` → `E--Projects-mcp-game-deck`); novo módulo `commands/sessions.rs` com (a) `get_sessions()` lista `*.jsonl`, parseia primeira `user` line pra título (filtrando `<command-message>...</command-message>` wrapper do init prompt do Claude Code), conta linhas user+assistant, mtime do file, sort desc por mtime; (b) `read_session_messages(id) -> Vec<Message>` parseia JSONL completo mapeando user/assistant linhas pra nossas Messages com Block[] (text/tool-use/tool-result), skip linhas system/permission-mode/file-history-snapshot/thinking; (c) `resume_session(id)` e `start_new_session()` falam com supervisor. **Supervisor** (`claude_supervisor/mod.rs`): novo field `resume_session_id: Arc<StdMutex<Option<String>>>` + setters/getters; re-push em respawn igual permission_mode (preserva escolha quando supervisor crash + auto-restart). `lib.rs` + `commands/mod.rs` registram os novos handlers. **JS** (`sdk_entry.js` template + runtime): `let pendingResumeSessionId = null`; handlers `{type:"setResumeSession",sessionId}` e `{type:"clearResumeSession"}` no stdin loop; `handleInput` passa `options.resume = pendingResumeSessionId` quando set (não limpa — turnos consecutivos no mesmo session continuam appendando). **TS**: `ipc/types.ts::SessionSummary`; `ipc/commands.ts::getSessions, resumeSession(id), startNewSession(), getSessionMessages(id)`. **Store** (`conversationStore.ts`): `currentSessionId` já existia, setter chamado nas ações; novo método `loadHistory(messages)` que substitui mensagens de uma vez. **UI**: novo `SessionList.tsx` faz fetch on mount, botão "New Session" no topo, lista de sessions (título + age relativa), highlight do session ativo; click handler chama `getSessionMessages(id)` + `loadHistory()` + `resumeSession(id)` + `setCurrentSessionId(id)`. `ChatRoute.tsx` virou sidebar layout (split 240px SessionList esquerda / chat direita). **VALIDADO:** smoke manual em Tauri+Unity: (1) sidebar renderiza sessões reais do `~/.claude/projects/E--Projects-mcp-game-deck/`; (2) **⭐ click em session antiga → chat carrega histórico visível** com blocks renderizados (text + tool-use + tool-result), não chat vazio; (3) continuar conversa após resume → contexto preservado, Claude lembra do que foi falado antes; (4) botão "New Session" cria session nova, sidebar atualiza, chat limpo; (5) zero resíduo de `.game-deck-sessions.json` no repo (já satisfeito pré-task). Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke (bug latente B4 sdk-entry stale cache). **Decision #6 da F02 confirmada empiricamente:** Claude Code session storage em `~/.claude/projects/` é source of truth; nosso próprio storage `.game-deck-sessions.json` não existe mais e não é necessário. Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. **GROUP 4 FECHADO** (4.1, 4.2, 4.3, 4.4 todos done). |
| 5.1 | Attachment migration — paths instead of base64 on the wire | M | ✅ | 2026-04-30 | Reconciliou o spec ("SDK handles encoding internally") com a realidade da SDK Anthropic Messages API: ImageBlockParam/DocumentBlockParam só aceitam base64/url sources, não path source. Encoding fica contido no boundary `sdk-entry.js` — paths fluem por toda parte (Tauri → JSON line stdin → JS), base64 nasce só no Node antes de chamar `query()`. Wire Tauri/React fica limpo, sem blobs. **JS side** (`sdk_entry.js` template + runtime mirror): helper `mediaTypeFor(ext)` mapeia .png/.jpg/.jpeg/.gif/.webp → image media types, .pdf → application/pdf, resto → null (vira text inline); helper async `buildAttachmentBlock(path)` lê via `fs.promises.readFile`, retorna ImageBlockParam (base64), DocumentBlockParam pdf (base64), ou DocumentBlockParam text (PlainTextSource utf8) — erro de leitura emite error envelope e retorna null; `handleInput(text, attachments)`: se attachments vazio mantém `prompt: text` string (caminho atual, zero overhead), se não-vazio constrói async generator `singleUserMessage(text, attachments)` yieldando 1 SDKUserMessage com content = [textBlock, ...attachmentBlocks] e passa como `prompt: asyncGen()` pra `query()`. **React side**: novo `src/hooks/useFileDragDrop.ts` faz wrap em `getCurrentWebview().onDragDropEvent(handler)` retornando `paths: string[]` no drop event; `routes/ChatRoute.tsx` ganhou local state `pendingAttachments: string[]`, drop handler appendizando, chip strip acima do textarea (filename + X remove com truncamento), drop highlight no container, submit passa array + clear; `stores/conversationStore.ts::sendMessage(text, attachments?: string[])` repassa pro `sendMessageCommand(text, attachments)` (wire pro Rust já assinado pra isso desde a 4.1). **Rust**: zero mudanças. **VALIDADO:** smoke manual em Tauri+Unity: (1) drag PNG (~200KB) + "what's in this image?" → Claude analisa image content corretamente; (2) drag PDF (~5MB) + "summarize this PDF" → Claude lê e resume; (3) DevTools network confirma payloads do `send_message` carregam paths como strings, não base64 blobs (encoding contido no boundary `sdk-entry.js`). Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke (bug latente B4 sdk-entry stale cache). **Cleanup #4 do spec** ("F01 base64 encoder code path is unused") vacuosamente true — F01 encoder vivia em `Editor/ChatUI/` (Unity-side, embedded UI legada), v2 React app foi scaffolded zerado sem essa machinery. Code path nunca existiu no v2. **Edge cases** (espaços, non-ASCII, deep nested) tratados implicitamente por `fs.promises.readFile` que aceita Windows paths nativos; validação manual fica pra task 5.2. Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 5.2 | Attachment cross-platform path test — Windows backslashes / spaces / non-ASCII | S | ✅ | 2026-04-30 | **Validation task** — zero código, smoke manual nos 4 cenários do spec. **Análise estática do path flow** (Claude Code dev): Tauri webview drag-drop → React state → sendMessage → Tauri command → supervisor → `serde_json::to_string` (UTF-8) → `tokio::stdin.write_all` (bytes) → Node `readline` (utf8 default) → `JSON.parse` → `path.extname` / `fs.promises.readFile`. Cadeia inteira UTF-8 byte-clean: Rust strings são sempre UTF-8, serde_json emite UTF-8 válido, tokio escreve bytes raw, Node readline default é utf8, NTFS suporta Unicode em filenames, `fs.promises.readFile` aceita Windows paths nativos. Conclusão prévia: zero normalização necessária. **VALIDADO** smoke manual em Tauri+Unity com PNG cópia replicado em 4 paths (PC pessoal, user `bedin`): (1) backslashes Windows `C:\Users\bedin\Desktop\test.png` → "what's in this image?" → Claude descreveu OK; (2) espaços `C:\Users\bedin\Desktop\my pictures\test.png` → "describe" → OK, sem erro de path; (3) non-ASCII `C:\Users\bedin\Desktop\maçã.png` → "describe" → OK, maçã com ç/ã no filename não quebrou parsing nem encoding; (4) deep nested + accent `C:\Projetos\Jurassic\Assets\Images\dinossauro.png` → "describe" → OK. **4/4 sem regressão.** Decisão de design ratificada: (a) **não** adicionar `process.stdin.setEncoding("utf8")` defensivo — redundante com default do `readline`; código defensivo sem causa polui codebase; (b) **não** adicionar normalize step de path no spawn.rs ou sdk_entry.js — desnecessário, fs.promises.readFile resolve Windows paths nativos. Risco residual improvável: Windows console codepage afetar stdin se Node spawn herdou codepage não-utf8 — não é nosso caso (readline default utf8 + Rust escreve bytes UTF-8 puros). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. **GROUP 5 FECHADO** (5.1, 5.2 todos done). |
| 6.1 | Health check on supervisor startup — minimal query, 5s timeout, surface clean failure | S | ✅ | 2026-04-30 | Implementação aplicada pelo Claude Code dev no PC pessoal antes da troca de máquina; validação concluiu no PC trabalho após hotfix do timeout legado. **Mudança semântica**: o sinal `{type:"ready"}` do JS deixou de transicionar o supervisor pra `Ready`. Em vez disso, agenda um health check 1.5s depois pra exercer o pipeline inteiro (módulo SDK importou + Anthropic API alcançável + auth válida + plugin loadable). Antes: Ready significava "JS booted; SDK module loaded". Depois: Ready significa "health check passou; pipeline funcional". **Rust**: novo `claude_supervisor/lifecycle.rs` com `schedule_health_check_trigger(stdin_tx)` helper isolado que sleep `HEALTH_CHECK_DELAY = 1500ms` + push do `{type:"healthCheck"}` no stdin. `types.rs::AgentMessage` ganhou variants `HealthOk` e `HealthFailed { message }` (kebab-case via serde existente). `claude_supervisor/spawn.rs::read_stdout`: nova signature ganhou 5º param `stdin_tx: mpsc::UnboundedSender<String>`; `AgentMessage::Ready` agora só loga "JS ready; scheduling health check in 1.5s" e dispara `tokio::spawn(lifecycle::schedule_health_check_trigger(stdin_tx_clone))` fire-and-forget; novos branches `HealthOk` → set `Ready` + emit changed; `HealthFailed { message }` → set `Crashed` + emit changed + log message. `claude_supervisor/mod.rs::spawn()` clona `tx` mais 1x e passa pro `read_stdout`. **JS** (`sdk_entry.js` source + runtime mirror): helpers `emitHealthOk()` / `emitHealthFailed(message)`; async `runHealthCheck()` chama `query({prompt:"__health__", options:{cwd:projectPath}})` (options mínimas, sem plugins/MCP/permissionMode pra isolar pipeline core), faz race contra `setTimeout(reject, 5000)` e loop interno `for await (const msg of q)` consome silenciosamente até receber `msg.type === "result"` (não emite text-delta nem turn-complete — chat fica intocado pelo health prompt); stdin loop ganha branch `{type:"healthCheck"}` → `void runHealthCheck()` (fire-and-forget pra não bloquear stdin loop). **TS**: `ipc/types.ts::AgentMessage` ganhou variants `health-ok` e `health-failed` (ChatRoute switch ignora silenciosamente, consistente com `permission-mode-changed` pattern). **Trade-off documentado**: o `__health__` query burns ~5k input + 10 output tokens reais por spawn (~$0.015 USD em Sonnet 4). Decisão consciente; alternativa `q.initializationResult()` em streaming-input mode (zero tokens, exige refactor pra streaming input em `handleInput`) catalogada no doc comment de `runHealthCheck()` em `sdk_entry.js` pra retake futuro se custo incomodar. **Hotfix do bug latente do timeout legado** (descoberto durante validação no PC trabalho): a mudança semântica da 6.1 (Ready persistir até health-ok chegar, ~6.5s no fluxo healthy) quebrou o invariante implícito do timeout legado da 2.2 em `mod.rs::spawn()` (o `tokio::spawn` final que faz `sleep + still_starting check + kill`). O timeout legado disparava em 5s encontrando status ainda `Starting` e marcava como `Failed` antes do health check completar, matando o supervisor toda spawn. Sintoma: log mostrava `JS ready; scheduling health check in 1.5s` seguido de `[sdk-entry] ready, projectPath=...` e então `ready signal timed out after 5s`. **Fix aplicado direto via filesystem MCP no PC trabalho** (não no plano original do Claude Code dev): removido o `tokio::spawn` final em `claude_supervisor/mod.rs::spawn()` (~30 linhas), removida a constante `READY_TIMEOUT: Duration` que ficou órfã, removido `use std::time::Duration` que era usado só por ela, doc comment de `spawn()` atualizado pra refletir nova semântica (Idle → Starting → health check → Ready/Crashed). A 6.1 já tem mecanismo próprio (`runHealthCheck` faz race contra `setTimeout 5s` e emite `health-failed` que vira `Crashed` via `read_stdout`) — manter os dois timeouts era redundante e o legado disparava antes do novo. **VALIDADO:** smoke manual em Tauri+Unity após hotfix: (1) healthy launch → `Idle` → `Starting` → `Ready` em ~7-9s (delay 1.5s + tempo do health check ~5s), sem mais "ready signal timed out"; (2) force fail mid-launch via Task Manager kill do Node child → status vai pra `Crashed` em até 5s + log `health check failed: ...`; (3) Restart Supervisor button (da 2.1) → fresh spawn → `Ready` de novo. Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke (bug latente B4 sdk-entry stale cache). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 6.2 | Crash detection — `node-sdk-status-changed: crashed` event wiring | S | ✅ | 2026-04-30 | Plano aplicado pelo Claude Code dev. **Rust** (pure Rust, zero mudanças TS/React): novo helper `claude_supervisor/lifecycle.rs::monitor_child_exit` que faz `child.wait().await` em background task spawn-and-forget, e quando o child morre transiciona supervisor pra `Crashed` + emite `supervisor-status-changed` event. **Idempotência para race conditions**: o monitor checa o status atual antes de transicionar — se já está `Crashed` ou `Failed`, no-op silencioso (evita evento duplicado quando health-failed da 6.1 já transicionou + child morre logo depois). **Discriminação shutdown intencional vs morte inesperada**: flag `intentional_shutdown: Arc<AtomicBool>` setada por `shutdown_inner` antes do `child.kill()`; monitor checa flag ao detectar exit — se setada, no-op (não dispara `Crashed`). **Owner do child**: `child.wait()` precisa de ownership exclusivo, mas `shutdown_inner` precisa de acesso pra `kill()`. Solução: `mod.rs::spawn()` faz `child.take()` do State, passa pro monitor task; shutdown_inner agora usa `Child::kill` via PID handle alternativo (signal-based) em vez de owned reference. **Param `emit_status` em `shutdown_inner`**: mantido pra a 6.3 preencher (clean shutdown on Tauri close vai precisar emitir `Idle` pra UI refletir antes do app fechar). **VALIDADO** smoke manual em Tauri+Unity nos cenários críticos do spec: (1) **⭐ cenário 2 (CRÍTICO — valida 6.2)**: após supervisor `Ready`, mata `node.exe` filho com `sdk-entry.js` no command line via Task Manager > Detalhes > Finalizar tarefa → badge do Supervisor virou vermelho `crashed` em < 2s + botão Restart Supervisor visível + log do terminal mostrou `[claude-supervisor] node child exited unexpectedly (code: ...)`; (2) cenário 3 (restart pós-crash) → click Restart Supervisor → spawn fresco → `Ready` em ~10-15s → prompt subsequente funcionou normal; (3) cenário 5 (restart sem crash prévio) → click Restart com supervisor `Ready` → status flow `Ready → Idle (silêncio) → Starting → Ready` sem evento `Crashed` espúrio no caminho (intentional_shutdown flag funcionou, monitor não disparou false-positive); (1) cenário 1 (happy path) → spawn limpo `Idle → Starting → Ready` sem `Crashed` espúrio. **Cenário 4 (race com health-failed)**: pulado por economia de tempo — idempotência validada implicitamente pelos cenários 2 e 5; sabotar `sdk_entry.js` com `process.exit(1)` 200ms após health-failed pra confirmar UM evento crashed apenas via DevTools pode ser feito futuramente em 7.x se concern aparecer. **Cenário 6 (window close + processos órfãos)**: parcial, escopo da 6.3 (clean shutdown). Não bloqueia 6.2 done. Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke quando template fonte mudou (bug latente B4). **Logs em inglês confirmados** (`eprintln!("[claude-supervisor] node child exited unexpectedly...")`) consistente com restante do módulo. Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. Bug latente registrado pra 7.x: timeout do health check da 6.1 pode precisar ser configurável ou adaptive (5s default original ficou inssuficiente no PC trabalho — cold start do `claude` binary excede 5s na primeira invocação; workaround atual é `HEALTH_CHECK_TIMEOUT_MS = 15000` em `sdk_entry.js`). |
| 6.3 | Clean shutdown — both Node child and claude subprocess terminate within 2s on Tauri close | M | ✅ | 2026-04-30 | Plano aplicado pelo Claude Code dev no PC pessoal (Jurassic). **Decisão central**: hard kill da árvore de processos sem nudge gracioso — budget de 2s torna round-trip de mensagem `{type:"shutdown"}` + abort + die incerto; SDK persiste sessão JSONL turn-by-turn então abrupção perde só turn em voo (que user aceitou ao fechar). **Estratégia por plataforma**: (Windows alvo prioritário) `taskkill /T /F /PID <node_pid>` via `tokio::process::Command` — `/T` mata árvore inteira incluindo claude grandchild + mcp-proxy; `/F` forçado sem WM_CLOSE; sem deps novas. (Unix) `#[cfg(unix)]` fallback ao `kill_on_drop` como rede de segurança — solução sólida com `process_group(0)` + `libc::kill(-pgid)` fica pra task separada quando alguém em macOS/Linux quiser. **Estado prévio leveraged**: `lib.rs::on_window_event(CloseRequested)` já fazia `prevent_close + tauri::async_runtime::spawn(async { shutdown.await; app.exit(0); })` desde F01 — async-em-sync handler já estava resolvido, só precisou validar budget 2s; gate `status==Idle` em `monitor_child_exit` (lifecycle.rs:98-110) já cobria intentional shutdown sem precisar flag explícita; `shutdown_inner` já fazia ordem correta (Idle → kill_tx). **Mudanças aplicadas**: `claude_supervisor/lifecycle.rs::monitor_child_exit` ganhou param `pid` e na branch `kill_rx` agora dispara `taskkill /T /F /PID <pid>` (Windows) ou `#[cfg(unix)]` deixa `start_kill` rodar normal antes do `wait()`, embrulhado em `tokio::time::timeout(Duration::from_millis(1500))` deixando ~500ms pro `wait()` reapar (total ≤ 2s). `claude_supervisor/mod.rs::spawn()` captura `child.id()` antes do move pro monitor task e passa como argumento. `claude_supervisor/mod.rs::shutdown_inner` ganhou `let start = Instant::now()` no início e `tokio::time::timeout(Duration::from_secs(2), monitor_handle).await` enforcement do budget — em timeout: log `[claude-supervisor] shutdown exceeded 2s budget — aborting monitor task; possible orphan` + `app.exit(0)` prossegue mesmo assim (preferimos sair do que travar). Final do shutdown_inner: `eprintln!("[claude-supervisor] shutdown completed in {:?}", start.elapsed())`. `claude_supervisor/spawn.rs::spawn_node_child` recebeu `kill_on_drop(true)` defensivo (rede de segurança caso shutdown_inner não seja chamado por algum motivo). **Lib.rs CloseRequested handler**: nenhuma mudança estrutural necessária — só validação que shutdown.await respeita budget. **Decisões sobre as 3 perguntas abertas**: (1) graceful nudge antes do hard kill = NÃO; (2) Unix agora ou só Windows = WINDOWS AGORA + `#[cfg(unix)]` kill_on_drop fallback; (3) Job Object Windows = TODO comment pra v2.1 (mais robusto mas requer crate `windows` + unsafe blocks). **VALIDADO** smoke manual em Tauri+Unity (PC pessoal Jurassic): (1) **⭐ steady state**: abrir Tauri, esperar Ready, fechar com X → shutdown completou em <500ms + Get-Process pós-close confirmou nenhum `node.exe` ou `claude.exe` órfão; (2) mid-turn: abrir, mandar prompt longo streamando, fechar com X → shutdown <2s, claude binário sumiu junto via taskkill /T; (3) mid-tool-call: prompt disparando MCP tool call do Unity, fechar enquanto tool em execução → shutdown <2s, tanto claude quanto mcp-proxy sumiram; (4) **5 ciclos abrir/fechar** com `Get-Process | Where-Object { $_.ProcessName -in 'node','claude' } | Select Id, ProcessName, Path` pós-cada-fechamento → zero acúmulo de órfãos pelo path filter (Path apontando pra App~/runtime/ ou ~/.claude/); (5) edge documentado: kill no PID do Tauri via Task Manager → órfãos esperados (handler não roda — Job Object resolveria, fora do escopo, registrado como TODO). **Resolve cenário 6 da 6.2** (window close + processos órfãos) que tinha ficado parcial. Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 6.4 | Windows hygiene — CREATE_NEW_PROCESS_GROUP equivalent + UTF-8 stdio | S | ✅ | 2026-04-30 | Plano aplicado pelo Claude Code dev no PC pessoal Jurassic. **Decomposição dos 3 entregáveis da spec**: (1) `CREATE_NEW_PROCESS_GROUP` no spawn do Node — implementado via novo módulo dedicado `claude_supervisor/windows_hygiene.rs::apply(&mut Command)` chamado em `spawn.rs::spawn_node_child` antes do `cmd.spawn()`. Usa `cmd.as_std_mut().creation_flags(0x0000_0200)` (constante hard-coded pra evitar dep no `windows`/`winapi` crate por um único inteiro). `as_std_mut()` em vez de tokio direct é defensiva contra version drift do tokio (Cargo.toml hoje pin só "1" major). `#[cfg(windows)]` no flag, `#[cfg(not(windows))]` no-op com `let _ = cmd;`. Módulo dedicado em vez de inline (escolha consciente confirmada): facilita expansão futura (console code page, locale env, terminal mode); spec pede literal. (2) UTF-8 stdio — `process.stdin.setEncoding('utf8')` adicionado no template fonte `src-tauri/src/claude_supervisor/sdk_entry.js` (não no runtime mirror, que é sobrescrito via include_str! — bug B4 conhecido). Defensiva pura: `readline.createInterface` default já é utf8 e `process.stdout.write(string)` em pipe já emite UTF-8; mudança explicita intenção sem custo. Lado Rust não precisou de mudança: `BufReader::lines()` já é UTF-8 e Rust strings são UTF-8 por construção. (3) `--append-system-prompt` via file path — **não acionável hoje**: F08 (Rules Page) ainda não existe e o caminho atual é `query({ options: { appendSystemPrompt: "..." } })` via SDK (string direta, não CLI arg — SDK abstrai a decisão internamente). Solução: comentário-âncora ADR-001 deixado in-situ em `sdk_entry.js` perto de `queryOptions` documentando a constraint pra quem implementar F08 ("prefer file-path reference over inline string for prompts >~8KB; long CLI args hit Windows arg-length limits and corrupt the SDK->claude handoff"). Sem mudança de comportamento. **Decisões sobre as 3 perguntas abertas**: (1) `runtime_setup` gera `sdk_entry.js` a cada launch = SIM (B4 conhecido), edita NO TEMPLATE FONTE; (2) comentário-âncora F08 em `sdk_entry.js` = OK, in-situ tem maior chance de ser visto que ADR em `docs/adr/`; (3) módulo dedicado vs inline = MÓDULO (10 linhas não é overkill, facilita expansão futura). **VALIDADO** smoke manual em Tauri+Unity (PC pessoal Jurassic): (1) **cenário Ctrl+C no PowerShell de `cargo tauri dev`**: comportamento real é Tauri morre por CTRL_C broadcast porque `cargo run` (parent) recebe primeiro — isso é **uma camada acima do nosso controle** (não é falha da 6.4, é problema do `cargo run` parent group, sem solução easy do nosso lado). Em produção (Tauri packaged exe lançado pelo OS sem console parent), CTRL_C_EVENT não existe — problema simplesmente não aparece. A 6.4 fez o que prometeu: o Node child ficou em process group próprio (verificado via `windows_hygiene.rs::apply` rodando antes do spawn). (2) UTF-8 round-trip: prompt "Escreva uma string Python contendo: 你好世界, café, naïve, 🚀, эхо" → chat renderizou todos os caracteres corretamente sem `?` nem mojibake (chars chineses, acentos PT/FR, emoji, cirílico). Eprintln no terminal podia mojibacar (PowerShell display issue, não data path). (3) regressão 6.3: smoke de close + `Get-Process | Where-Object { $_.ProcessName -in 'node','claude' }` pós-fechamento confirmou zero órfãos — `taskkill /T /F` não usa CTRL_C_EVENT (TerminateProcess), então `CREATE_NEW_PROCESS_GROUP` não interfere. **Limitação documentada**: em dev mode (`cargo tauri dev` num PowerShell), Ctrl+C ainda derruba a stack inteira (`cargo run` morre, Tauri morre junto). Solução seria isolar Tauri exec em outro grupo, mas isso é trabalho do framework Tauri ou packaging — fora do escopo do supervisor. Workaround pra dev: usar X da janela em vez de Ctrl+C. Workaround conhecido aplicado: `runtime/sdk-entry.js` apagado manualmente antes do smoke (B4). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 6.5 | Pin known-good Claude Code version range; warn-but-don't-block on drift | S | ✅ | 2026-04-30 | Plano aplicado pelo Claude Code dev no PC pessoal Jurassic. **3 entregáveis decompostos**: (1) **Range no `package.json`**: campo `"claudeCode": ">=2.1.0 <3.0.0"` adicionado em repo-root — doc viva pra desenvolvedores. Range escolhido com base em **realidade testada** (claude `--version` local no PC trabalho era 2.1.126) em vez da sugestão aspiracional `>=2.10.0` da spec original — preferimos honestidade sobre realidade testada do que aspiração (não fizemos smoke em 2.10.x). Unity Package Manager ignora campo desconhecido (sem efeito colateral). (2) **Source-of-truth do range em Rust**: hardcoded em constantes em novo módulo `claude_supervisor/version_check.rs` — `MIN_VERSION: (u32, u32, u32) = (2, 1, 0)` e `MAX_VERSION_EXCL: (u32, u32, u32) = (3, 0, 0)`. Decisão consciente vs `include_str!("package.json")` + parse JSON: path com 4 níveis fragíl, parse compile-time exigiria `serde_json` em `build.rs` (infra desproporcional), risco de divergência baixo (claudeCode é doc human-readable, pior caso de descuido é banner errado não corrupção). Doc-comments em ambos os lados (constantes em Rust + campo no `package.json`) explicitam que se mirroram. **Comparator pequeno**: `parse_version(raw) -> Option<(u32, u32, u32)>` strip `v` prefix, strip suffix após `-` ou `+` (pre-release/build metadata), split por `.`, parse cada segmento como u32, faltando segmento defensivamente vira 0 (ex: "2.10" -> (2,10,0)). Comparação tupla-a-tupla via `PartialOrd` derivado. 12 testes unitários cobrindo casos comuns + boundaries inclusive/exclusive + pre-release + edge cases (garbage, two segments, just-below upper, local dev version). (3) **Override pra teste**: `MCP_GAME_DECK_CLAUDE_VERSION_OVERRIDE` env var em `install_check::detect_claude_version` short-circuita o probe e retorna a string verbatim — permite exercer banner sem instalar build out-of-range; não consumida por nenhum código de produção. **(4) `--append-system-prompt` via file path**: deixado de fora (já abordado pela 6.4 como comentário-âncora ADR-001 em `sdk_entry.js`; F08 ainda não existe). **Quando rodar a verificação**: task fire-and-forget em `lib.rs::run`'s setup, em paralelo ao spawn do supervisor (probe é independente do caminho crítico de boot do supervisor). **Evento Tauri**: `EVT_CLAUDE_VERSION_OUT_OF_RANGE = "claude-version-out-of-range"` em `events.rs` + helper `emit_claude_version_out_of_range`; `ClaudeVersionOutOfRangePayload { detected, supported }` em `types.rs` (camelCase). **Banner React**: novo `components/ClaudeVersionWarningBanner.tsx` espelhando o pattern do `UpdateBanner` — subscribe via `onClaudeVersionOutOfRange` em useEffect, dismissable per-session (vai aparecer de novo no próximo boot — versão drift pode ter resolvido entre boots, intencional), faixa âmbar com texto "Claude Code v{detected} detected; tested with {supported}. Some features may behave differently." + botão × dismiss. Mounted em `App.tsx` perto do `<UpdateBanner />`. **Hotfix do bug latente de race condition de timing** (descoberto durante validação no PC pessoal Jurassic): o evento `claude-version-out-of-range` era emitido pelo Rust ~instantâneo após `lib.rs::run::setup` (pq `version_check::run` não tem caminho crítico de boot longo — detect_claude_version com override é instantâneo), mas o React `ClaudeVersionWarningBanner` só monta depois do `App.tsx` clarear `FirstRunCheckingScreen` (que aguarda `checkClaudeInstallStatus()` ~5s de probes paralelos). Tauri events são fire-and-forget e não-buffered, então evento se perdia silenciosamente — log `[version-check]` aparecia mas banner não renderizava na UI. **Fix aplicado direto via filesystem MCP no PC pessoal**: `version_check.rs` ganhou constante `EMIT_DELAY: Duration = Duration::from_secs(6)` e `tokio::time::sleep(EMIT_DELAY).await` antes do `emit_claude_version_out_of_range` — dá tempo do React clarear FirstRunCheckingScreen e montar o banner antes do evento chegar. Doc-comment expandido explicando o trade-off (banner aparece com 6s de atraso, mas é advisory não-blocking, app fica usable nesse meio tempo). Solução ideal seria buffering no Rust + fetch inicial no React (novo Tauri command `get_claude_version_warning()` retornando `Option<Payload>`), mas isso seria mais código pra spec da 6.5 que é "Small" — atraso de 6s é fix focado e bem documentado. **Decisões sobre as 3 perguntas abertas**: (1) range = realidade testada `>=2.1.0 <3.0.0`; (2) banner UX = per-session dismiss alinhado com UpdateBanner pattern; (3) field name = `claudeCode` literal da spec (vai aparecer em outros docs futuros como README disclosure da 7.2, LICENSE; trocar criaria divergência entre código e doc). **VALIDADO** smoke manual em Tauri+Unity (PC pessoal Jurassic): (1) **⭐ cenário in-range silencioso**: boot normal sem override → sem banner, sem evento `claude-version-out-of-range` no DevTools console (regressão zero); (2) **⭐ cenário out-of-range banner** com `$env:MCP_GAME_DECK_CLAUDE_VERSION_OVERRIDE = "2.0.5"` no MESMO PowerShell antes de `pnpm tauri dev` → log apareceu em ~1s `[version-check] claude 2.0.5 outside supported range >=2.1.0 <3.0.0; surfacing banner in 6s`, **banner âmbar apareceu no topo da janela ~6s depois** após `FirstRunCheckingScreen` clarear, com texto "Claude Code v2.0.5 detected; tested with >=2.1.0 <3.0.0. Some features may behave differently." + botão × dismissvel; app continuou funcional, manda prompt e recebe response normal. Cenários edge da spec (boundaries 3.0.0/2.1.0, pre-release suffix, garbage "abc") cobertos pelos 12 unit tests do `parse_version` (ver `version_check.rs::tests`). Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. **GROUP 6 FECHADO** (6.1, 6.2, 6.3, 6.4, 6.5 todos done). |
| 7.1 | E2E smoke test — F07 pin → Tauri → 3-turn conversation w/ tool call + attachment, repeat 5x | M | ✅ | 2026-04-30 | **Marcada done com base em validação incremental já feita ao longo dos Groups 4-6, não em smoke E2E formal repetido aqui.** A spec da 7.1 decompõe-se em 4 elementos, todos já cobertos: (1) **Multi-turn conversation (3 turnos)** — validado na 4.1 com prompt "what year is it now? and the next year?" cujo contexto foi preservado entre turnos; também repetidamente validado na 4.4 (resume session + continue conversation) e em todos os smokes que envolveram conversa de múiltiplos turnos pra reproduzir bugs. (2) **Tool call funcionando** — validado na 4.1 (`mcp__game-deck__scene-list` via permission ask), 4.2 (`mcp__game-deck__scene-list` via bypassPermissions sem ask), 3.4 (`mcp__game-deck__console-get-logs` via ToolSearch + invocation), 6.4 (regressão `taskkill /T /F` não quebrou tool calls). (3) **Attachment funcionando** — validado na 5.1 (drag PNG ~200KB "what's in this image?" + drag PDF ~5MB "summarize" + DevTools network confirmou paths como strings não base64 blobs) e na 5.2 (4 paths Windows com edge cases: backslashes, espaços, non-ASCII `maçã.png`, deep nested + accent `dinossauro.png`). (4) **Repeat 5x pra detectar bugs intermitentes** — validado na 6.3 (5 ciclos abrir/fechar Tauri com `Get-Process | Where-Object { $_.ProcessName -in 'node','claude' }` pós-cada-fechamento, zero acumulação de órfãos, zero regressão). **Elemento NUNCA validado**: F07 pin no Editor Unity abrindo Tauri. **Justificativa pra não validar agora**: o pin do F07 é mecanismo de **release production** — ele aponta pra Tauri exe instalado/empacotado, não pro dev mode (`pnpm tauri dev`/`cargo tauri dev` que rodam target/debug direto). Em desenvolvimento, sempre lançamos Tauri via dev mode bypassando o pin. Validar pin → Tauri-installed-exe só faz sentido durante **release prep da v2.0** quando tivermos: (a) Tauri release build empacotado via `cargo tauri build`, (b) installer/distribúível real, (c) F07 pin configurado com path do exe instalado em vez do target/debug. **Feedback do user**: "mas ja vemos fazendo isso, pq validar denovo na 7.1? ja testamos cara, até agora tudo certo!" — razável: re-rodar smoke E2E formal com 3-turn + tool call + attachment + 5x já explicitamente cobertos seria duplicação desnecessária. Trade-off aceito conscientemente. **Registrado pra release prep v2.0**: smoke da 7.1 com Tauri-release-exe + F07 pin completo deve rodar quando empacotamento estiver pronto, junto com sweep do README (1.1.0 → 2.0.0) e atualização de screenshots. Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 7.2 | README + LICENSE proprietary-dependency disclosure (or track as TODO) | S | ✅ | 2026-04-30 | **Aplicado disclosure compacto** no README via filesystem MCP, **sem mexer no LICENSE** (mantém MIT puro, disclosure separado é melhor prática). Nova seção `Third-Party Dependencies` adicionada entre `Requirements` e `License`, cobrindo: (1) **Claude Code** — Anthropic's CLI agent, não bundled, link pra docs + Anthropic Usage Policy + Commercial Terms of Service; (2) **@anthropic-ai/claude-agent-sdk** — usado pela Chat UI, mesmas terms. Texto final explicita que MIT cobre **apenas** o source authored aqui; user é responsável por instalar Claude Code separadamente + auth Anthropic própria + compliance com terms. **Decisão de design**: README já mencionava Claude Code várias vezes (header "Requires Claude Code & Node.js", seção Prerequisites com link, seção Requirements), então disclosure poderia ter virado redundância. Solução: seção dedicada e formal, focada na **separação de licenças** (protegência legal) em vez de re-explicar a dependência. **Não mexido no LICENSE**: prática estándard é manter LICENSE puro com a license escolhida (MIT) e adicionar disclosures em README ou NOTICE separado. Inflar LICENSE com texto de terceiros confunde grep automatizado de license scanners. **Observação do user**: README do projeto ainda passará por sweep maior antes do release v2.0 (atualizar version number 1.1.0 → 2.0.0, novas features F02/F07, novas screenshots, etc). O disclosure aplicado agora é self-contained e não precisará de retake durante esse sweep — fica como base. Pre-existente: F07 erro do agent-sdk-stub continua até 7.3. |
| 7.3 | Cleanup — remove dead JSON-RPC types from F01 era | S | ✅ | 2026-04-30 | Plano aplicado pelo Claude Code dev no PC pessoal Jurassic. **Inventory prévio rigoroso** levantou 5 itens Rust + 5 React mortos (todo o code path JSON-RPC da F01, superseded pelo Claude Code Agent SDK supervisor da F02). **Removido (Rust)**: (a) `App~/src-tauri/src/node_supervisor/{mod.rs, jsonrpc.rs, protocol.rs, spawn.rs}` — 4 arquivos via `Remove-Item` no Bash; (b) `types::NodeSdkStatus` enum + `types::NodeSdkStatusChangedPayload` struct; (c) `events::EVT_NODE_SDK_STATUS_CHANGED` const + `emit_node_sdk_status_changed` helper (já marcados `#[allow(dead_code)]` desde F02); (d) `events::EVT_NODE_LOG` const — só emitido de dentro de `node_supervisor/jsonrpc.rs`; depois do delete não tem mais emitter; (e) `commands::dev::node_ping` fn — estava broken em runtime (`NodeSupervisor::new()` não estava em `lib.rs::run::manage(...)` desde F02; chamar `nodePing()` do React resultaria em panic "state not managed"); (f) entry de `invoke_handler!` pro `node_ping` em `lib.rs`; (g) `pub mod node_supervisor;` em `lib.rs`. **Removido (TS/React)**: (a) `ipc/commands.ts::nodePing` export; (b) `ipc/events.ts::onNodeLog` wrapper + import `NodeLogPayload`; (c) `ipc/types.ts::NodeLogPayload` interface; (d) `App.tsx`: useEffect inteiro do `onNodeLog` (forwardava log do Node SDK pra DevTools console — não existe mais Node SDK F01-style; sdk-entry.js da F02 emite typed AgentMessage events em vez de log lines); (e) `routes/SettingsRoute.tsx`: botão "Ping Node SDK" + state vars (`pingResult`, `pinging`) + `handlePing` handler + import `nodePing`. **Doc-comments atualizados** (4 referências órfãs): `lib.rs` (doc do módulo + doc do `run()`), `events.rs` ("kept while node_supervisor still emits" + "replaces node-sdk-status-changed from F01"), `unity_client/mod.rs` ("// same pattern as NodeSupervisor::status"), `claude_supervisor/install_check.rs` (referência a `node_supervisor::spawn::resolve_stub_script`). **Bullets da spec já cumpridos por trabalho anterior** (verificado via grep, zero matches): F01 base64 attachment encoder (vivia em `Editor/ChatUI/` Unity-side, v2 React app foi scaffolded zerado sem essa machinery), `get_conversation_history` / `clear_conversation` (apagados na 4.1), `sdk-entry.js` "single roundtrip" path (substituído por streaming na 4.1), `App~/src-tauri/src/jsonrpc/` top-level (não existia; `jsonrpc.rs` vivia dentro de `node_supervisor/` e morreu junto). **Decisões sobre as 4 perguntas abertas**: (1) `NodeLogPayload` em types.ts = REMOVE (grep confirmou só events.ts e App.tsx consomem; ambos saem junto); (2) botão "Ping Node SDK" no Settings = SÓ REMOVE (não tem equivalente óbvio no claude_supervisor; health check já roda automaticamente no spawn da 6.1); (3) `Remove-Item` via Bash = SIM (escopo cirúrgico, VS Code Source Control mostra tudo antes do commit pra revisão); (4) smoke pós-cleanup = compile-clean (`cargo check` + `cargo test` + `tsc --noEmit`) cobre o critério da 7.3 ("remove dead code"); validação funcional foi coberta pelas tasks anteriores. **VALIDADO**: `cargo check` zero erros + zero warnings novos; `cargo test` todos os tests existentes passam (incluindo os 12 unit tests do `parse_version` da 6.5); `pnpm tsc --noEmit` zero erros TS. Grep de safety pós-cleanup zero matches pra `node_supervisor`, `NodeSdkStatus`, `node_ping`, `onNodeLog`, `NodeLogPayload`, `EVT_NODE_LOG`, `EVT_NODE_SDK_STATUS_CHANGED` no `App~`. Smoke confidence-builder: abrir Tauri 1 vez, espera Ready (~10s), confere badge verde em Settings, fecha pelo X — sem regressão, sem erros no terminal. **GROUP 7 FECHADO** (7.1 marcada done por validação incremental, 7.2 disclosure no README, 7.3 cleanup). **🎯 FEATURE 02 (CLAUDE CODE SUPERVISOR) FECHADA INTEGRALMENTE — 24/24 TASKS DONE.** Pre-existente F07 erro do agent-sdk-stub registrado: pendente de cleanup paralelo no F07 (não bloqueia F02 done). |
