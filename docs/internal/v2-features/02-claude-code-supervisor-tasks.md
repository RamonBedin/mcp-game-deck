...
| 2.2 | Spawn Agent SDK with cwd + envs from F07 launch contract; smoke round-trip | M | ⏳ | | |
| 2.3 | Agent SDK message stream → React conversationStore (text deltas only) | M | ⏳ | | |
| 2.4 | Agent SDK message stream — tool use blocks + tool result blocks | M | ⏳ | | |
