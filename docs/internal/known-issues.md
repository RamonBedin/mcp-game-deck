...
**Surprise during smoke test (added to scope).** After the three fixes above landed and the NSIS installer was tested end-to-end, the proxy still failed: `ERR_MODULE_NOT_FOUND: Cannot find package '@modelcontextprotocol/sdk'`. Root cause: the proxy was shipped as a single `.js` file via Tauri Resource bundling, but its imports (`@modelcontextprotocol/sdk` and friends) live in `Server~/node_modules/` — which doesn't ship alongside the file. In v1.0 / dev mode this never surfaced because the proxy ran from `Server~/dist/mcp-proxy.js` where Node could walk up to `Server~/node_modules/`. Resolution: **esbuild bundles the proxy as a self-contained CJS file** (single 700KB output that inlines all JS deps). New `bundle:proxy` script in `Server~/package.json` runs `esbuild --bundle --platform=node --format=cjs --target=node20`.

**Second surprise during dev-mode validation.** Day-after fix worked in the NSIS install but failed in `pnpm tauri dev` with `ReferenceError: require is not defined in ES module scope`. The CJS bundle was named `.js`, which relies on Node defaulting to CommonJS when no parent `package.json` declares `"type"`. That assumption held for the NSIS install path (`%LOCALAPPDATA%\MCP Game Deck\proxy-bundle\mcp-proxy.js` — no `package.json` above it). It broke for the dev path (`App~/src-tauri/target/debug/proxy-bundle/mcp-proxy.js` — walks up into `App~/package.json` with `"type": "module"`, which forces ESM and crashes the CJS bundle). Resolution: **rename output extension to `.cjs`**. Node treats `.cjs` as CommonJS unconditionally, regardless of the parent `package.json` lookup chain. Touched: `Server~/package.json` `bundle:proxy` script, `App~/scripts/build-proxy.mjs`, `App~/src-tauri/tauri.conf.json` `bundle.resources` entry, and `paths::mcp_proxy_script` resolve string.
